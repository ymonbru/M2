...
def toTactic (t : rwTriangle α) : TacticM <| TSyntax `tactic := withMainContext do
  ...
  match t.dep with
    | none => match t.dir with
      | false => `(tactic | conv_lhs =>  rw [($proofTerm)] )
      | true => `(tactic | conv_lhs => rw [← $proofTerm] )
    | some pos =>
...
-/
def expand? (t : triangle α) (head : α) (before after oldState endState: List α) (explored : HashMap (List α) (Option <| (List α) × rwTriangle α)) (storedStates : Queue (List α)) : Except (HashMap (List α) (Option (List α × rwTriangle α))) (Queue (List α) × HashMap (List α) (Option <| (List α) × rwTriangle α)) :=
  if t.h == head then
    let newState := before ++ t.f :: t.g :: after
    let (unchanged?, newExplored) := explored.containsThenInsertIfNew newState (some ⟨oldState, t, none⟩)
...
def apply?  (t : triangle α) (head1 head2 : α) (pos : Nat) (before after oldState endState: List α) (explored : HashMap (List α) (Option <| (List α) × rwTriangle α)) (storedStates : Queue (List α)): Except (HashMap (List α) (Option (List α × rwTriangle α))) (Queue (List α) × HashMap (List α) (Option <| (List α) × rwTriangle α)) :=
  if t.f == head1 ∧ t.g == head2 then
    let newState := before ++ t.h :: after
    let (unchanged?, newExplored) := explored.containsThenInsertIfNew newState (some ⟨oldState, t, some pos⟩)
    if newState == endState then
      Except.error newExplored
    else
      if unchanged? then
...
def nextTriangle? (pos : Nat) (before curentState oldState endState: List α) (explored : HashMap (List α) (Option <| List α × rwTriangle α) ) (t : triangle α) (storedStates : Queue (List α)) : Except (HashMap (List α) (Option (List α × rwTriangle α))) (Queue (List α) × HashMap (List α) (Option <| (List α) × rwTriangle α)) :=  match curentState with
  | [] => Except.ok (storedStates, explored)
  | [a] => do
...
def explore (state endState: List α) (explored : HashMap (List α) (Option <| (List α) × rwTriangle α) ) (lt : List (triangle α)) (storedStates : Queue (List α)) : Except (HashMap (List α) (Option (List α × rwTriangle α))) (Queue (List α) × HashMap (List α) (Option <| (List α) × rwTriangle α)) := match lt with
  | [] => Except.ok (storedStates, explored)
  | t :: q => do
...
partial def BFS_step (stateStored : Queue (List α)) (lt : List (triangle α)) (endState : List α) (explored : HashMap (List α) (Option <| List α × rwTriangle α)) : HashMap (List α) (Option <| List α × rwTriangle α) :=
...
partial def findPathAux (explored : Std.HashMap (List α) (Option (List α × rwTriangle α))) (state : List α) (currentPath : List <| TSyntax `tactic): TacticM (List <| TSyntax `tactic) := do
    let previous := explored[state]?
