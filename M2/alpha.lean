...
def V (h: K.carrier ⊆ U.carrier) :Opens X:= sorry

#check V X K U _

lemma intermedKU_spec (h: K.carrier ⊆ U.carrier): K.carrier ⊆ (V _ K U h).carrier ∧ closure (V _ K U h).carrier ⊆ U.carrier ∧ IsCompact (closure (V _ K U h).carrier):= sorry
