...
--ordered set with a notion of inf
...
lemma selfSubThickening : ∀ e (A : Set X), A ⊆ thickening A e := sorry

/- A smaller thickening is included in a bigger one-/
lemma isMonotonusEpsThickening : ∀ e f, e ≤ f →  ∀ A : Set X, thickening A e ⊆ thickening A f  := sorry

lemma isMonotonusSetThickening : ∀ e (A B : Set X), A ⊆ B → thickening A e ⊆ thickening B e := sorry
...
lemma cupMonotonusthinckening : ∀ e (A B : Set X), thickening (A ∪ B) e ⊆ thickening A e ∪ thickening B e := sorry


variable [TopologicalSpace X]
/- A thickening is always open-/
lemma isOpenThickening : ∀ e (A : Set X), IsOpen <| thickening A e := sorry
...
example : ∃ V1 V2 : Set X, IsOpen V1 ∧ IsOpen V2 ∧ K1 ⊆ V1 ∧ K2 ⊆ V2 ∧ V1 ∩ V2 ⊆ U := by
  ...
  let e2 : Eps := (disjointThickenings A B hA hB hAB).choose
  let he : Disjoint (thickening A e2) (thickening B e2):= (disjointThickenings A B hA hB hAB).choose_spec
