...
\begin{proof}
    %Let $V$ be an open neighbourhood of $f^{-1}(K)$. Because $X$ is locally comapct Hausdorf, let $W$ be an open neighbourhood of $f^{-1}(K)$ relatively compact such that $\overline{W}\subset V$. Then $K\subset f(\overline{W})$, but $f$ is proper so it's closed, then $f(\overline{W})$ is a closed subset of $Y$, hence relatively compact hausdorff.\\

    %Then $K$ has a relatively compact open neighborhood $U$ (in $Y$)such that  $K\subset U\subset \overline{U}\subset f(\overline{W})$. Then one gets $f^{-1}(K)\subset f^{-1}(U)\subset \overline{W}\subset V$. wich conclude the proof.
\end{proof}
